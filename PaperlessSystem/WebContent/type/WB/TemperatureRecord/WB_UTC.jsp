...
<html>
	...
	<script type="text/javascript">
		var CheckboxHandler = function(id) {
			$("#tbl").show();
			$(":input[name='Item08']").prop('disabled',false);
			$("#Item03, #Item04, #Item05, #Item06, #Item07, #Item09, #Item10").prop('disabled', false);
			if(id==='select1' && $("#select1").prop('checked')) {
				...
				$("#select1, #select5").prop('checked', false);
					...
					$("#tbl").hide();
					$(":input[name='Item08']").prop('checked',false);
					$(":input[name='Item08']").prop('disabled',true);
					$("#select1, #select2, #select3, #select4").prop('checked', false);
					$("#Item02, #Item03, #Item04, #Item05, #Item06, #Item07, #Item09, #Item10").val('');
					$("#Item02, #Item03, #Item04, #Item05, #Item06, #Item07, #Item09, #Item10").prop('disabled', true);
				}
			}
		}
	</script>
	...
	<script type="text/javascript">
		...
		var ValidationHandler = function() {
			...
			/* 個別バリデーション処理 */
			// 計画停止は個別バリデーションの対象外
			if ($("#select5").prop('checked')) { $(":input[name='Item08']").prop('checked',false); submit(); return; }
			if (!$.isNumeric($("#Item09").val())) {
			...
			var msg='';
			var num = $("#Item09").val();
			var val = $("input[name='Item08']:checked").val();
			if (!val) {
			...
			if(0<msg.length) { $("#Item11").val('×'); msg += '【判定】判定結果が×です。<br/>'; }
			else { $("#Item11").val('○'); }
	...
	<head>
		...
		<title>UTC ワイヤボンダ作業温度実測記録</title>
	</head>

	<body>
		<div class="block">ＵＴＣ　ワイヤボンダ作業温度実測記録</div>
		<jsp:include page="/include/include_pagetop.jsp"/>
		<form id="fm" action="<%=request.getContextPath()%>/TenkenListServlet" method="post">
			<input type="hidden" name="sheetnm" value="ＵＴＣ　ワイヤボンダ作業温度実測記録">
						...
						<td colspan=2>
							<table class="table table-hover" id="tbl" >
								...
								<tr>
									...
									<td>
										<div class="btn_group_80 week" style="margin : 3.5px 0px 0px 0px">
								...
								<tr>
									<td rowspan=4>温度計指示値</td><td colspan=2>PRE</td>
								...
								<tr>
									...
									<td>
										<div class="btn_group_180" style="margin : 3.5px 0px 0px 0px">
											<input type="radio" name="Item08" id="radio①" value="①140～160℃">
											<label for="radio①">①140～160℃</label>
											<input type="radio" name="Item08" id="radio②" value="②155～165℃">
											<label for="radio②">②155～165℃</label>
											<input type="radio" name="Item08" id="radio③" value="③160～180℃">
											<label for="radio③">③160～180℃</label>
											<input type="radio" name="Item08" id="radio④" value="④190～210℃">
											<label for="radio④">④190～210℃</label>
											<input type="radio" name="Item08" id="radio⑤" value="⑤220～240℃">
											<label for="radio⑤">⑤220～240℃</label>
											<input type="radio" name="Item08" id="radio⑥" value="⑥230～250℃">
											<label for="radio⑥">⑥230～250℃</label>
											<input type="radio" name="Item08" id="radio⑦" value="⑦250～270℃">
								...
								<tr>
									<td colspan=4>判定</td>
									<td><input type="text" id="Item11" class="input RoundedCorners ShadowInsetTop" readonly="readOnly" style="margin:10px 0px 0px 0px;background-color:#FFFFFF"></td>
								</tr>
